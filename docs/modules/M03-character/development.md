...
人格分成核心preset、可学习候选与当前状态。Core组装对话时从当前preset读取稳定身份，从云记忆读取带证据和版本的关系摘要与自身经历；学习候选未经用户接受不写入preset。候选保存状态proposed/accepted/rejected、evidence_ids、origin_character_version；接受后走正常配置发布和设备ACK。

按 [分层记忆规格](../../12-人格关系与分层记忆系统.md) 验证拒绝不合理设定跳变、关系误推断、长期不互动、角色版本切换和故事归属。
...
reflection使用companion_reflection来源、character_world和inferred；持久人格变化沿用学习候选的接受流程。核心预设与反思通道分离，候选证据变更立即失效。 共同约束见 [13 记忆实现方案](../../13-Ombre-Brain参考与记忆实现方案.md)。
